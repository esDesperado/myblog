...
const Calculator = observer((data,v) => {
    ...
    if(inface.components.length > 0 && JSON.stringify(calc).length === 2){
        ...
        let obj2 = calc
        let obj3 = multiplier
        inface.components.slice().sort((a,b)=>a.priority - b.priority).map((pr,key)=>{if(pr.type === data.id+'calcProp'){
            if(pr.price === 'checkbox' && !obj2[pr.title]){
                inface.components.slice().sort((a,b)=>a.priority - b.priority).map((d,key)=>{if(d.type === data.id+'pr'+pr.id && !obj2[pr.id]){
                    obj1[pr.id] = 0
                    obj2[pr.id] = []
                    obj3[pr.id] = 1
                }})
    ...
    useEffect(()=>{
        let str1 = ''
        inface.components.slice().sort((a,b)=>a.priority - b.priority).map((pr,key)=>{if(pr.type === data.id+'calcProp' && calc[pr.id]){
            str1 += '<br/><b>' + pr.title + ':</b><br/>'
            inface.components.slice().sort((a,b)=>a.priority - b.priority).map((d,key)=>{if(d.type === data.id+'pr'+pr.id){
    ...
    <div ref={block} id={'block'+data.id} className='block' style={{overflow:'hidden',position:'relative'}}>
        ...
        <div style={{padding:'80px 7% 80px 7%',position:'relative',width:'86%',display:'grid',gridTemplateColumns:'1fr 1fr',background:data.shadow?(data.background || inface.interface.background) + data.shadow:data.background || 'transparent',color:data.color || inface.interface.color,}}>
            <div className={JSON.parse(data.obj || "{}").anim?'FDAStart':'FDAEnd'} style={{gridColumn:'1/3',fontSize:'1em',textAlign:'center',marginBottom:inface.width > 1000?'30px':'20px',fontWeight:'500',position:'relative'}}>
                {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<input onClick={()=>document.getElementById('title'+'I'+data.id).style.display = 'block'} style={{fontWeight:'bold',background:inface.aback,color:inface.acolor,padding:'10px 30px',display:!data.title || !data.title.length?'inline-block':'none'}} id={'title'+'B'+data.id} value='Заголовок' type='button'/>:null}
                <span onClick={()=>{if(user.role > 3){document.getElementById('title'+'I'+data.id).style.display = 'block'}}} style={{fontSize:inface.width > 1000?'2.5em':'1.5em',margin:'0',cursor:'default',}} dangerouslySetInnerHTML={{__html:data.title}}/>
                {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<textarea rows='1' onBlur={e=>{document.getElementById('title'+'I'+data.id).style.display = 'none';setBAttr('title',e.target.value,data.id).then(d=>{inface.setBlocks(d)}).catch(d=>alert(d))}} id={'title'+'I'+data.id}  style={{fontWeight:'bold',display:'none',position:'absolute',bottom:'0px',color:inface.acolor,background:inface.aback,border:'1px '+inface.acolor+' solid',left:inface.width > 400?'calc(50% - 300px)':'0',width:'600px',maxWidth:'100%',fontSize:'1.5em'}}/>:null}
            </div>
            <div className={JSON.parse(data.obj || "{}").anim?'FDAStart':'FDAEnd'} id={'calculator'+data.id} style={{display:'grid',gridTemplateColumns:inface.width > 1000?'50% 50%':'100%',width:'88%',background:(data.color || inface.interface.color)+'11',border:'1px '+(data.color || inface.interface.color)+'22 solid',borderRadius:'10px',gridColumn:'1/3',padding:'6%',}}>
                <div style={{width:'100%',marginBottom:'15px'}}>
                    ...
                    <div style={{width:'100%',fontSize:inface.width > 1000?'1em':'0.9em',display:'inline-block',margin:inface.width > 1000?'0 0 5px 0px':user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?'0':'0 0px',position:'relative',}}>
                        <div onClick={()=>{if(calc.type !== pr.title && user.role === 0){if(inface.components.filter((a,key)=>a.type === data.id+'calcProp' && a.title === pr.title)[0].price.includes('*')){let obj1 = multiplier;obj1.type = parseFloat(inface.components.filter((a,key)=>a.type === data.id+'type' && a.title === pr.title)[0].price.replace('*',''));setMultiplier(obj1)}else{let obj1 = multiplier;if(obj1.type !== 1){obj1.type = 1;setMultiplier(obj1)}};let obj = JSON.parse(JSON.stringify(calc));obj.type = pr.title;setCalc(obj);}}} style={{position:'relative',width:'100%',overflow:'hidden',cursor:'pointer',marginBottom:'8px'}}>
                            {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<input onClick={()=>document.getElementById('title'+'CI'+pr.id).style.display = 'block'} style={{width:'100%',fontWeight:'bold',background:inface.aback,color:inface.acolor,padding:'0px 30px',marginBottom:'3px',display:!pr.title || !pr.title.length?'inline-block':'none'}} id={'title'+'CB'+pr.id} value='Название свойства' type='button'/>:null}
                            {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<input type='text' onBlur={e=>{document.getElementById('title'+'CI'+pr.id).style.display = 'none';setCAttr('title',e.target.value,data.id,pr.id).then(d=>{inface.setComponents(d)}).catch(d=>alert(d))}} id={'title'+'CI'+pr.id}  style={{fontWeight:'400',display:'none',position:'absolute',top:'0',color:inface.acolor,background:inface.aback,border:'1px '+inface.acolor+' solid',left:'0',width:'400px',maxWidth:'100%',zIndex:'3',fontSize:'0.9em'}}/>:null}
                            <div style={{fontSize:'1.1em',fontWeight:'500',cursor:'default',display:'inline-block',textAlign:'left'}} onClick={()=>{if(user.role > 3){document.getElementById('title'+'CI'+pr.id).style.display = 'block'}}}>{pr.title}</div>
                        </div>
                            ...
                            {inface.components.slice().sort((a,b)=>a.priority - b.priority).map((d,key)=>d.type === data.id+'pr'+pr.id &&
                                <div style={{width:'100%',fontSize:inface.width > 1000?'1.1em':'1em',display:'inline-block',margin:inface.width > 1000?'0px':user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?'0':'0 0px',position:'relative',}}>
                                    <div onClick={()=>{if(calc[pr.id] !== d.id && (user.role === 0 || !document.location.pathname.includes(ADMIN_ROUTE))){let obj1 = multiplier;let obj2 = addPrice;if(d.price){if(d.price.includes('*')){obj1[pr.id] = parseFloat(d.price.replace('*',''));obj2[pr.id] = 0;}else{if(obj1[pr.id] !== 1){obj1[pr.id] = 1;};obj2[pr.id] = parseInt(d.price)}}else{if(obj1[pr.id] !== 1){obj1[pr.id] = 1;};obj2[pr.id] = 0};setMultiplier(obj1);setAddPrice(obj2);let obj = JSON.parse(JSON.stringify(calc));obj[pr.id] = d.id;setCalc(obj);}}} style={{position:'relative',width:'100%',overflow:'hidden',cursor:'pointer'}}>
                                        {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<input onClick={()=>document.getElementById('title'+'CI'+d.id).style.display = 'block'} style={{width:'100%',fontWeight:'bold',background:inface.aback,color:inface.acolor,padding:'0px 30px',marginBottom:'3px',display:!d.title || !d.title.length?'inline-block':'none'}} id={'title'+'CB'+d.id} value='Вариант' type='button'/>:null}
                                        {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<input defaultValue={d.title} type='text' onBlur={e=>{document.getElementById('title'+'CI'+d.id).style.display = 'none';setCAttr('title',e.target.value,data.id,d.id).then(d=>{inface.setComponents(d)}).catch(d=>alert(d))}} id={'title'+'CI'+d.id}  style={{fontWeight:'400',display:'none',position:'absolute',top:'0',color:inface.acolor,background:inface.aback,border:'1px '+inface.acolor+' solid',left:'0',width:'400px',maxWidth:'100%',zIndex:'3',fontSize:'0.9em'}}/>:null}
                                        ...
                                        <div className='hoverOp' style={{padding:'4px 0',textDecoration:'none',color:data.color || inface.interface.color,width:'100%',cursor:'pointer'}} onClick={()=>{if(user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)){document.getElementById('title'+'CI'+d.id).style.display = 'block'}}}>{d.title}</div>
                                    </div>
                                    <div style={{position:'relative'}}>
                                        {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<input onClick={()=>document.getElementById('price'+'CI'+d.id).style.display = 'block'} style={{fontWeight:'bold',background:inface.aback,color:inface.acolor,padding:'0px 30px',marginBottom:'3px',display:!d.price || !d.price.length?'inline-block':'none'}} id={'price'+'CB'+d.id} value='Стоимость' type='button'/>:null}
                                        {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<input defaultValue={d.price} type='text' onBlur={e=>{document.getElementById('price'+'CI'+d.id).style.display = 'none';setCAttr('price',e.target.value,data.id,d.id).then(d=>{inface.setComponents(d)}).catch(d=>alert(d))}} id={'price'+'CI'+d.id}  style={{fontWeight:'400',display:'none',position:'absolute',top:'0',color:inface.acolor,background:inface.aback,border:'1px '+inface.acolor+' solid',left:'0',width:'400px',maxWidth:'100%',zIndex:'3',fontSize:'0.9em'}}/>:null}
                                        {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE) && <div className='hoverOp' style={{textDecoration:'none',color:data.color || inface.interface.color}} onClick={()=>{if(user.role > 3){document.getElementById('price'+'CI'+d.id).style.display = 'block'}}} style={{cursor:'pointer'}}>{d.price}</div>}
                                    </div>
                                    {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<div style={{marginTop:inface.width > 600?'0px':'0'}}>
                            ...
                            {inface.components.slice().sort((a,b)=>a.priority - b.priority).map((d,key)=>d.type === data.id+'pr'+pr.id &&
                                <div style={{width:'100%',cursor:'pointer',fontSize:inface.width > 600?'1.1em':'1em',display:'inline-block',margin:inface.width > 600?'0 0px':user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?'0':'0 0px',position:'relative',}}>
                                    <div onClick={()=>{if(user.role === 0 || !document.location.pathname.includes(ADMIN_ROUTE)){let obj1 = JSON.parse(JSON.stringify(calc));let obj2 = JSON.parse(JSON.stringify(addPrice));let obj3 = JSON.parse(JSON.stringify(multiplier));if(obj1[pr.id]){if(!obj1[pr.id].includes(d.id)){if(d.price){if(d.price.includes('*')){obj3[pr.id] += parseFloat(d.price.replace('*','')) - 1;}else{obj2[pr.id] += parseInt(d.price)}}else{if(obj3[pr.id] !== 1){obj3[pr.id] = 1;};obj2[pr.id] = 0};;obj1[pr.id].push(d.id)}else{if(d.price.includes('*')){obj3[pr.id] -= parseFloat(d.price.replace('*','')) - 1}else{obj2[pr.id] -= parseInt(d.price)};obj1[pr.id] = obj1[pr.id].filter(da=>da !== d.id)}};setCalc(obj1);setAddPrice(obj2);setMultiplier(obj3);}}} style={{position:'relative',width:'100%',overflow:'hidden',cursor:'pointer',padding:'2px 0'}}>
                                        {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<input onClick={()=>document.getElementById('title'+'CI'+d.id).style.display = 'block'} style={{width:'100%',fontWeight:'bold',background:inface.aback,color:inface.acolor,padding:'0px 30px',marginBottom:'3px',display:!d.title || !d.title.length?'inline-block':'none'}} id={'title'+'CB'+d.id} value='Название' type='button'/>:null}
                                        {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<input defaultValue={d.title} type='text' onBlur={e=>{document.getElementById('title'+'CI'+d.id).style.display = 'none';setCAttr('title',e.target.value,data.id,d.id).then(d=>{inface.setComponents(d)}).catch(d=>alert(d))}} id={'title'+'CI'+d.id}  style={{fontWeight:'400',display:'none',position:'absolute',top:'0',color:inface.acolor,background:inface.aback,border:'1px '+inface.acolor+' solid',left:'0',width:'400px',maxWidth:'100%',zIndex:'3',fontSize:'0.9em'}}/>:null}
                                        ...
                                        <div className='hoverOp' style={{textDecoration:'none',color:data.color || inface.interface.color}} onClick={()=>{if(user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)){document.getElementById('title'+'CI'+d.id).style.display = 'block'}}} style={{width:'100%',cursor:'pointer'}}>{d.title}</div>
                                    </div>
                                    <div style={{position:'relative'}}>
                                        {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<input onClick={()=>document.getElementById('price'+'CI'+d.id).style.display = 'block'} style={{fontWeight:'bold',background:inface.aback,color:inface.acolor,padding:'0px 30px',marginBottom:'3px',display:!d.price || !d.price.length?'inline-block':'none'}} id={'price'+'CB'+d.id} value='Стоимость' type='button'/>:null}
                                        {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<input defaultValue={d.price} type='text' onBlur={e=>{document.getElementById('price'+'CI'+d.id).style.display = 'none';setCAttr('price',e.target.value,data.id,d.id).then(d=>{inface.setComponents(d)}).catch(d=>alert(d))}} id={'price'+'CI'+d.id}  style={{fontWeight:'400',display:'none',position:'absolute',top:'0',color:inface.acolor,background:inface.aback,border:'1px '+inface.acolor+' solid',left:'0',width:'400px',maxWidth:'100%',zIndex:'3',fontSize:'0.9em'}}/>:null}
                ...
                <div style={{fontWeight:'500',fontSize:'1.4em',gridColumn:inface.width > 1000?'1/3':'1/2'}}>
                    <div style={{position:'relative',minWidth:'50%',textAlign:'left'}}>
                        {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<input onClick={()=>document.getElementById('map'+'I'+data.id).style.display = 'block'} style={{fontWeight:'bold',position:'absolute',top:'-10px',display:'inline-block',background:inface.aback,color:inface.acolor,padding:'0 10px',display:!data.text3 || !data.text3.length?'inline-block':'none'}} id={'map'+'B'+data.id} value='стоимость' type='button'/>:null}
                        <div onClick={()=>{if(user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)){document.getElementById('map'+'I'+data.id).style.display = 'block'}}} style={{fontSize:'0.8em',fontWeight:'500',padding:'0',}} dangerouslySetInnerHTML={{__html:data.map?data.map.replace(/\n/g, "<br/>"):''}}/>
                        {user.role > 3 && document.location.pathname.includes(ADMIN_ROUTE)?<textarea rows='1' onBlur={e=>{document.getElementById('map'+'I'+data.id).style.display = 'none';setBAttr('map',e.target.value,data.id).then(d=>{inface.setBlocks(d)}).catch(d=>alert(d))}} id={'map'+'I'+data.id} style={{fontSize:'1em',fontWeight:'bold',top:'0px',display:'none',position:'absolute',color:inface.acolor,background:inface.aback,width:'100%',border:'1px '+inface.acolor+' solid',minWidth:'200px',}}/>:null}
                    </div>
